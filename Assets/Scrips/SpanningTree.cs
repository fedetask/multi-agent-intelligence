...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class SpanningTree : MonoBehaviour
    {
        ...
        List<Vector3> dfs_recurs(int[] root, int index)
        {
            List<Vector3> result = new List<Vector3>();

            string node = array_to_string(root, index);
            List<int[]> children = spanning_trees[node];
            result.Add(coord_to_vec(root));
            if (children.Count == 0) //base of recursion
            {
                return result;
            }
            else
            {
                ...
                {
                    result.AddRange(dfs_recurs(child, index)); //add the subtree
                    result.Add(coord_to_vec(root)); //you came back, add yourself before you move on to the next subtree
                }
                return result;
            }
